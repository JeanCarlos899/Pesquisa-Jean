...
\begin{columns}[T,totalwidth=\textwidth]
\column{.47\textwidth}
{\Large\bfseries O que já existia}

\vspace{.35cm}
\begin{tikzpicture}[node distance=.32cm]
  \node[draw=ufpiBlue!35,fill=white,rounded corners=7pt,text width=5.2cm,inner sep=8pt] (a)
    {\bfseries CRIC Cervix\\{\small\color{muted}imagens cervicais com pontos}};
  \node[below=.25cm of a,draw=ufpiBlue!35,fill=white,rounded corners=7pt,text width=5.2cm,inner sep=8pt] (b)
    {\bfseries Detecção em citologia\\{\small\color{muted}modelos para localizar células}};
  \node[below=.25cm of b,draw=ufpiBlue!35,fill=white,rounded corners=7pt,text width=5.2cm,inner sep=8pt] (c)
    {\bfseries Supervisão por pontos\\{\small\color{muted}ponto como pista espacial fraca}};
\end{tikzpicture}

\column{.53\textwidth}
{\Large\bfseries O que faltava fechar}

\vspace{.35cm}
\begin{tikzpicture}[
  flowstep/.style={draw=ufpiBlue!45,fill=white,rounded corners=6pt,
    minimum width=1.48cm,minimum height=.9cm,align=center,font=\small},
  arr/.style={-{Latex[length=2mm]},lineGray,thick}
]
  \node[flowstep] (p) at (0,0) {Pontos};
  \node[flowstep] (d) at (1.88,0) {Detector};
  \node[flowstep] (l) at (3.76,0) {Localiza\\células};
  \node[flowstep] (c) at (5.64,0) {Conta\\por imagem};
  \draw[arr] (p) -- (d);
  \draw[arr] (d) -- (l);
  \draw[arr] (l) -- (c);
\end{tikzpicture}
...
\vspace{.45cm}
\begin{itemize}
  \item Treinar a partir de pontos sem perder o vínculo com a célula real.
  \item Avaliar localização e contagem juntas, não só uma métrica isolada.
  \item Confirmar a escolha em um teste retido.
\end{itemize}

\vspace{.15cm}
{\small\color{muted}Este trabalho mede esse caminho completo na CRIC Cervix.}
\end{columns}
\end{frame}
