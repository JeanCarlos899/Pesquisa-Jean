...
def public_path_text(value: str) -> str:
    root = Path.cwd().resolve()
    text = value
    windows_root = str(root).rstrip("\\") + "\\"
    escaped_root_ascii = json.dumps(windows_root, ensure_ascii=True)[1:-1]
    escaped_root_utf8 = json.dumps(windows_root, ensure_ascii=False)[1:-1]
    for prefix in [windows_root, escaped_root_ascii, escaped_root_utf8]:
...
def sanitize_public_values(table: pd.DataFrame) -> pd.DataFrame:
    data = table.copy()
    for column in data.select_dtypes(include=["object"]).columns:
        data[column] = data[column].map(lambda value: public_path_text(value) if isinstance(value, str) else value)
    return data


def sanitize_public_json(value):
    if isinstance(value, dict):
        return {key: sanitize_public_json(item) for key, item in value.items()}
...
def write_table(table: pd.DataFrame, target: Path, index: bool = False) -> None:
    table = sanitize_public_values(table)
...
def copy_if_exists(source: Path, target: Path) -> bool:
    if source.exists():
        target.parent.mkdir(parents=True, exist_ok=True)
        if source.suffix.lower() in {".csv", ".json", ".md", ".tex"}:
            target.write_text(public_path_text(source.read_text(encoding="utf-8")), encoding="utf-8")
        else:
...
def make_materials(config: Config, output_dir: str | Path = "results/article_materials") -> None:
    out = Path(output_dir)
    figures = out / "figuras"
    tables = out / "tabelas"
...
def write_summary(config: Config, metrics: Path) -> None:
    ...
    summary = sanitize_public_json(summary)
...
def write_readme(out: Path) -> None:
    lines = [
        "# Materiais do artigo",
        "",
        "Pasta gerada automaticamente a partir dos artefatos locais de avaliacao.",
        "",
        "## Figuras",
    ]
